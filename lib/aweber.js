...
function RequestWrapper(args, apiCall) {
    ...
    if(!Array.isArray(this.args[0]) && apiCall.paramNames.length > 0){
...
RequestWrapper.prototype.request = function(callBack, envOptions) {
    ...
    Aweber._core.makeApiRequest(env, callBackWrapper, this.args, this.apiCall);
...
Aweber._core = (function() {
    ...
    coreRef.makeApiRequest = function(env,callBack,args,apiData) {
    ...
    function getApiURL(env,args,apiData) {
        ...
        for ( var i = 0 ; i < apiData.paramNames.length; i++){
            var reg = new RegExp(':'+apiData.paramNames[i]);
            urlPrefix = urlPrefix.replace(reg,args[0][i]);
        }
